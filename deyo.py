    ...
    def forward(self, x, iter_, targets=None, flag=True, group=None):
        ...
        if targets is None:
            for _ in range(self.steps):
                if flag:
                    if self.args.Add_TCA:
                        embeddings, outputs, backward, final_backward = forward_and_adapt_deyo(x, iter_, self.model, self.args,
                    ...
                    if self.args.Add_TCA:
                        embeddings, outputs = forward_and_adapt_deyo(x, iter_, self.model, self.args,
                                                        self.optimizer, self.deyo_margin,
                                                        self.margin_e0, targets, flag, group)
                    else:
            ...
            for _ in range(self.steps):
                if flag:
                    if self.args.Add_TCA:
                        embeddings, outputs, backward, final_backward, corr_pl_1, corr_pl_2 = forward_and_adapt_deyo(x, iter_, self.model, 
                                                                                                        self.args, 
                                                                                                        self.optimizer, 
                                                                                                        self.deyo_margin,
                                                                                                        self.margin_e0,
                    ...
                    if self.args.Add_TCA:
                        embeddings, outputs = forward_and_adapt_deyo(x, iter_, self.model, 
        ...
        if targets is None:
            if flag:
                if self.args.Add_TCA:
                    return embeddings, outputs, backward, final_backward
                else:
            ...
            if flag:
                if self.args.Add_TCA:
                    return embeddings, outputs, backward, final_backward, corr_pl_1, corr_pl_2
                else:
...
def forward_and_adapt_deyo(x, iter_, model, args, optimizer, deyo_margin, margin, targets=None, flag=True, group=None):
    ...
    with torch.no_grad():
        ###############################################################
        set_record_feature(model, record_feature=False)
        ###############################################################

        ############################ TCA ############################
        if args.Add_TCA:
            _, outputs_prime = model(x_prime)
        else:
