...
class EATA(nn.Module):
    ...
    """
    def __init__(self, args, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False, e_margin=math.log(1000)/2-1, d_margin=0.05):
        super().__init__()
        self.model = model
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                if self.args.Add_TCA:
                    embeddings, outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.args, self.model, self.optimizer,
                    ...
                    outputs, num_counts_2, num_counts_1, updated_probs = forward_and_adapt_eata(x, self.args, self.model, self.optimizer,
                    self.fishers, self.e_margin, self.current_model_probs, fisher_alpha=self.fisher_alpha, num_samples_update=self.num_samples_update_2, d_margin=self.d_margin)
...
def forward_and_adapt_eata(x, args, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, scale_factor=2, num_samples_update=0):
    ...
    if x[ids1][ids2].size(0) != 0:
        loss.backward()
        ###############################################################
        if args.plugin_mixtta and args.eta > 0:
            filter_feature_record(model, filter_ids_1)
            if filter_ids_2 is not None:
    ...
    if args.Add_TCA:
        return embeddings, outputs, entropys.size(0), filter_ids_1[0].size(0), updated_probs
    else:
